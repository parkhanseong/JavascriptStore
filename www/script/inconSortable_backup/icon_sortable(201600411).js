	...
	for (var i = 0; i < Object.keys(data.items).length; i++) {
		...
		if (data.items[i].groupSeq == 0) {
			...
			html += "<div class=\"item icon\" data-seq=\"" + data.items[i].seq
					...
					+ "\" data-sort=\"" + data.items[i].sort + "\"data-sort=\""
					+ data.items[i].sort + "\" data-groupSeq=\""
					+ data.items[i].groupSeq + "\">" + data.items[i].title
					+ "</div>";
				...
				for (var k = 0; k < data.items.length; k++) {
					...
					if (typeof data.groups[i] === 'undefined') {
						...
						html += "<p data-seq=\"" + data.items[k].seq
								...
								+ "\"data-sort=\"" + data.items[k].sort
								+ "\" data-groupSeq=\""
								+ data.items[k].groupSeq + "\">"
